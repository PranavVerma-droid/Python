...
def extract_questions_and_code(tex_file):
    ...
    # Regex to match \subsection*{number. Question} and \begin{verbatim} code \end{verbatim}
    pattern = re.compile(r"\\subsection\*\{(\d+)\. .*?\}\s*\\begin\{verbatim\}(.*?)\\end\{verbatim\}", re.S)
...
def update_tex_with_image(tex_file, question_number, image_path):
    ...
    # Regex to find the correct subsection
    subsection_pattern = rf"(\\subsection\*\{{{question_number}\. .*?\\end\{{verbatim\}})"
    match = re.search(subsection_pattern, tex_content, re.S)

    if match:
        # Add the \includegraphics command after the \end{verbatim}
...
def main():
    tex_file = r"C:\Users\prana\Github\Languages\Python3\School\Class XI\Practical File(s)\Finals\Final Practical File.tex"
