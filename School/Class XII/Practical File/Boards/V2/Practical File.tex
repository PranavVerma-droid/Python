...
\begin{enumerate}
    ...
    \item[(vi)] SELECT DISTINCT Type FROM Furniture;
    \item[(vii)] SELECT MAX(PRICE) FROM Furniture WHERE DateofStock > '2002-02-15' ;
    \item[(viii)] SELECT COUNT(*) FROM Furniture WHERE Discount < 25 ;
\end{enumerate}
...
(vii)
\begin{lstlisting}[language=SQL]
SELECT MAX(PRICE) FROM Furniture WHERE DateofStock > '2002-02-15' ;
\end{lstlisting}

(viii)
\begin{lstlisting}[language=SQL]
SELECT COUNT(*) FROM Furniture WHERE Discount < 25 ;
\end{lstlisting}
...
\begin{enumerate}
    \item[(i)] List the names of those students who have obtained rank 1 sorted by NAME.
    \item[(ii)] Display a list of all those names whose AVERAGE is greater than 65.
    \item[(iii)] Display the names of those students who have opted COMPUTER as a SUBJECT with an AVERAGE of more than 60.
    \item[(iv)] List the names of all the students in alphabetical order.
    \item[(v)] SELECT * FROM GRADUATE WHERE NAME LIKE "\%I\%";
    \item[(vi)] SELECT DISTINCT RANK FROM GRADUATE;
\end{enumerate}
...
(v)
\begin{lstlisting}[language=SQL]
SELECT * FROM GRADUATE WHERE NAME LIKE "%I%";
\end{lstlisting}
...
\begin{enumerate}
    \item[(i)] Write SQL query to create the table.
    \item[(ii)] Write SQL query to increase the size of SNAME to hold 30 characters.
    \item[(iii)] Write SQL query to remove the column HOBBY.
    \item[(iv)] Write SQL query to insert a row in the table with any values of your choice that can be accommodated there.
\end{enumerate}
...
\begin{enumerate}
    \item[(i)] Write SQL Query to display ProductName and Price for all products whose Price is in the range 50 to 150.
    \item[(ii)] Write SQL Query to display details of products whose manufacturer is either XYZ or ABC.
    \item[(iii)] Write SQL query to display ProductName, Manufacturer and Price for all products that are not giving any discount.
    \item[(iv)] Write SQL query to display ProductName and price for all products whose ProductName ends with 'h'.
    \item[(v)] Write SQL query to display ClientName, City, P\_ID and ProductName for all clients whose city is Delhi.
    \item[(vi)] Which column is used as Foreign Key and name the table where it has been used as Foreign key?
\end{enumerate}
...
(vi) The column used as a Foreign Key is P\_ID in the CLIENT table, and it references the P\_ID column in the PRODUCT table.
...
\begin{enumerate}
    \item[(a)] To list the names of all the patients admitted after 1998-01-15.
    \item[(b)] To list the names of female patients who are in ENT department.
    \item[(c)] To list the names of all patients with their date of admission in ascending order.
    \item[(d)] To display Patient's Name, Charges, Age for only female patients.
    \item[(e)] Find out the output of the following SQL commands:
    \begin{itemize}
        \item SELECT COUNT(DISTINCT Charges) FROM HOSPITAL;
        \item SELECT MIN(Age) FROM HOSPITAL WHERE Sex = "F";
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item[(a)] Identify the attribute best suitable to be declared as a primary key.
    \item[(b)] Write the degree and cardinality of the table STORE.
    \item[(c)] Insert the following data into the attributes ItemNo, ItemName and SCode respectively in the given table STORE.

    ItemNo = 2010, ItemName = "Note Book" and Scode = 25.
    \item[(d)] Abhay wants to remove the table STORE from the database MyStore. Which command will he use from the following?
    \begin{itemize}
        \item DELETE FROM STORE;
        \item DROP TABLE STORE;
        \item DROP DATABASE MYSTORE;
        \item DELETE STORE FROM MYSTORE;
    \end{itemize}
    \item[(e)] Now Abhay wants to display the structure of the table STORE, i.e., name of the attributes and their respective data types that he has used in the table. Write the query to display the same.
\end{enumerate}
